...
# ╔═╡ d246a3e2-e925-11ed-2037-4def986b1eea
using Plots, ControlSystemsBase, RobustAndOptimalControl, PlutoUI

# ╔═╡ e11f4a57-a677-41c5-b13b-742744d1a14b
P = zpk([], [-4.86, 4.85], 4.6)

# ╔═╡ f4f631a9-6a43-407a-ade2-e8c2483d7580
pzmap(P,markeralpha=1,markerstrokewidth=3,markersize=7) # one unstable pole p=4.85, fundamental limit-> closed loop crossover frequency > 2*p 

# ╔═╡ f7d8dc63-a7fa-4d4d-a347-54467d7d5224
w = exp10.(LinRange(-2.5, 3, 500))

# ╔═╡ 90cbc0ee-68de-4d73-a83e-2e66d986c58b
function pid_marginplot(C)
	f1 = marginplot(P*C, w) # open loop
	vline!([2*4.85], label="Fundamental limitation", l=(:dash, :black), legend=:bottomleft)
	ylims!((1e-3, 1e2), subplot=1)	
	#ylims!((-180,0), subplot=2)
	f2 = nyquistplot(P*C, xlims=(-10,1), ylims=(-2,10), title="")
	plot(f1,f2,titlefontsize=10)
end

# ╔═╡ 9fba6e7c-4e72-416f-8946-4d4075ae94b2
begin
	slider_kp = Slider(0.1:0.1:30, show_value=true, default=1)
	slider_Ti = Slider(0.0:0.05:5, show_value=true, default=0)
	slider_Td = Slider(0.0:0.01:0.5, show_value=true, default=0)
	slider_Tf = Slider(0.0:0.001:0.02, show_value=true, default=0)
end;

# ╔═╡ 7884df6b-472d-46a0-bf64-bd77597a3b15
md"""
``k`` = $(@bind k slider_kp)\
``T_i`` = $(@bind Ti slider_Ti)\
``T_d`` = $(@bind Td slider_Td)\
``T_f`` = $(@bind Tf slider_Tf)
"""

# ╔═╡ e0310f20-cd1d-4d90-b98e-4a3de6f93060
C = pid(k, Ti, Td; Tf)

# ╔═╡ 17106d00-7b1d-46f5-ad41-c80a1cf102b9
pzmap(feedback(P*C),markeralpha=1,markerstrokewidth=3,markersize=7,xlims=(-20,20),ylims=(-10,10))

# ╔═╡ 8fcb94a3-6c2e-48d8-a0ed-2f0326b1ae06
pid_marginplot(C)

# ╔═╡ 7439646f-38de-4f61-8e5e-f36d6429eb57
isstable(feedback(P*C))

# ╔═╡ 704c0200-9d34-47f4-b1a3-074c879851e0
plot(step(feedback(P*C), 8, method=:zoh), ylabel="Φe") # closed loop step response

# ╔═╡ 6628624e-08cd-44e1-8da8-ef411d71da92
plot(step(extended_gangoffour(P,C)[:,1], 8, method=:zoh), ylabel=["ϕe" "u"])

# ╔═╡ f9c6e39c-ee8c-472a-8bc9-439119ffcbf2
plot(lsim(extended_gangoffour(P,C)[:,1], (x,t)->min(t,1), 8, method=:zoh), ylabel=["ϕe" "u"])
...
[compat]
ControlSystemsBase = "~1.4.2"
Plots = "~1.38.11"
PlutoUI = "~0.7.51"
RobustAndOptimalControl = "~0.4.24"
"""
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "8231b8b4519c368f5187ac59cbcbe328bd2cbb89"
...
git-tree-sha1 = "697d430bd2cf2ac9f13d10a02452ede406f02c75"
...
git-tree-sha1 = "3dca90bbcbc4c9a44ce302a07b19e6145931e485"
...
git-tree-sha1 = "dae68101f9c0836b2d079f4f2ff7773bc8b5d30d"
...
git-tree-sha1 = "d32f0725966e31fb274b80e59735e86cfac83afc"
...
git-tree-sha1 = "2ea3818e94945a210d0744248cdaa58e36bf5c73"
